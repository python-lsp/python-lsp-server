...
def test_pylint(config, workspace) -> None:
    with temp_document(doc_text=DOC, workspace=workspace) as doc:
        diags = pylint_lint.pylsp_lint(
            config=config, workspace=workspace, document=doc, is_saved=True
        )
...
def test_lint_free_pylint(config, workspace) -> None:
    ...
    ws = Workspace(
        root_uri=str(Path(__file__).absolute().parents[2]), endpoint=workspace._endpoint
    )
    assert not pylint_lint.pylsp_lint(
        config=config,
        workspace=ws,
        document=Document(uri=uris.from_fs_path(__file__), workspace=ws),
        is_saved=True,
    )
