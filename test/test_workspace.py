...
def test_multiple_workspaces(tmpdir, pylsp):
    ...
    added_workspaces = [{"uri": path_as_uri(str(x))} for x in (workspace1_dir, workspace2_dir)]
...
def test_progress_with_percent(workspace, consumer):
    with workspace.report_progress("some_title", "initial message", percentage=1) as progress_message:
