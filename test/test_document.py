...
def test_document_empty_edit(workspace):
    ...
    doc.apply_change({"range": {"start": {"line": 0, "character": 0}, "end": {"line": 0, "character": 0}}, "text": "f"})
    assert doc.source == "f"
...
def test_document_line_edit(workspace):
    ...
    doc.apply_change(
        {"text": "goodbye", "range": {"start": {"line": 0, "character": 3}, "end": {"line": 0, "character": 8}}}
    )
...
def test_document_multiline_edit(workspace):
    ...
    doc.apply_change(
        {"text": "print a, b", "range": {"start": {"line": 1, "character": 4}, "end": {"line": 2, "character": 11}}}
    )
...
def test_document_end_of_file_edit(workspace):
    old = ["print 'a'\n", "print 'b'\n"]
    doc = Document("file:///uri", workspace, "".join(old))
    doc.apply_change({"text": "o", "range": {"start": {"line": 2, "character": 0}, "end": {"line": 2, "character": 0}}})
    assert doc.lines == [
        "print 'a'\n",
        "print 'b'\n",
        "o",
