...
def __empty_identation_stack(identation_stack, level_limits, current_line, folding_ranges):
...
def __compute_folding_ranges_identation(text):
    ...
    for i, line in enumerate(lines):
        ...
        if identation_match is not None:
            ...
            folding_ranges = __empty_identation_stack(identation_stack, level_limits, current_line, folding_ranges)
            current_level = 0
        if line.strip() != "":
            current_line = i
    folding_ranges = __empty_identation_stack(identation_stack, level_limits, current_line, folding_ranges)
...
def __check_if_node_is_valid(node):
    ...
    if isinstance(node, tree_nodes.PythonNode):
        kind = node.type
        valid = kind not in {"decorated", "parameters", "dictorsetmaker", "testlist_comp"}
        if kind == "suite":
